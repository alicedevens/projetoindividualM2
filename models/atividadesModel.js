...
class Atividades {
  ...
  static async getAtividadesById(id) {
    const result = await db.query(
      "SELECT * FROM atividades WHERE id_atividades = $1",
      [id]
    );
  ...
  static async getAtividadesToDo() {
    const result = await db.query(
      "SELECT * FROM atividades WHERE status = FALSE"
    );
    return result.rows[0];
  }
  static async getAtividadesDone() {
    const result = await db.query(
      "SELECT * FROM atividades WHERE status = TRUE"
    );
    return result.rows[0];
  }
  ...
  static async delete(id) {
    const result = await db.query(
      "DELETE FROM atividades WHERE id_atividades = $1 RETURNING *",
      [id]
    );
